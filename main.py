...
try:
    ...
    print(f'[Warning] Missing lib: {e}')
    VIDEO_ENGINE = 'none'
    np = None; cv2 = None; Image = None
...
print(f"[Startup] Video engine: {VIDEO_ENGINE}")
if cv2: print(f"[Startup] OpenCV: {cv2.__version__}")
...
async def make_cv_video(images: List[str], p: dict, jid: str) -> str:
    ...
    if VIDEO_ENGINE == 'none' or cv2 is None or Image is None:
        print('[make_cv_video] OpenCV/Pillow not available')
        return ""
